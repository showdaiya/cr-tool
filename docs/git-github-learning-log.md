...
**説明**: `git checkout <branch>` を実行すると、実際のファイル（ワーキングディレクトリ）がそのブランチの状態に書き換わる。Gitは各ブランチのファイル内容を`.git`フォルダ内に保存しており、checkoutでどのバージョンを展開するか切り替える仕組み。

**イメージ**:
```
.git/（内部）
├── main の状態
├── feat/new-feature の状態
└── fix/bug の状態

git checkout feat/new-feature
↓
ワーキングディレクトリのファイルが feat/new-feature の内容に変わる
```

**なぜ別ブランチの変更が `npm run dev` に反映される？**:
- `npm run dev` は単純に「現在のフォルダにあるファイル」を読み込む
- checkoutでファイルが書き換わるので、開発サーバーにも反映される
- Next.jsのTurbopackは変更を検知して自動リロード
