...
## 7) 学習ログ（Learning Logs）
学習目的のため、各技術の知識を**カテゴリ別**に記録する：

**記録先**:
| ファイル | 対象 |
|----------|------|
| `docs/command-learning-log.md` | PowerShell, cmd |
| `docs/git-github-learning-log.md` | Git, GitHub, GitHub Copilot, MCP |
| `docs/aws-learning-log.md` | S3, CloudFront, OAC, IAM, CLI |
| `docs/typescript-learning-log.md` | 型定義, ジェネリクス, ユーティリティ型 |
| `docs/nextjs-learning-log.md` | App Router, Server/Client Components, Hooks |
| `docs/playwright-learning-log.md` | E2Eテスト, セレクタ, アサーション |

**フォーマット（カテゴリ別 + 統合アップデート）**:
```markdown
## [カテゴリ名]（例: Git - ブランチ操作）
...
**関連リンク**: [公式ドキュメント](https://...)
```

**提案タイミング（トリガー）**:
- コマンド（PowerShell/cmd/Git/AWS CLI）やAWS手順を提示・実行した直後
- テスト（Playwright含む）やCI設定に触れた直後
- PR作成/コミット/マージの直前
- 作業完了時の締め（"学習ログ更新しますか？"）

**実行方法**:
- 更新の実作業は skill **`update-learning-logs`** を使って行い、対象ログが複数ファイルにまたがる場合もまとめて更新する

**記録タイミング**: 作業完了時、またはユーザーが「ログを書いて」と指示したとき

**構成ルール**:
- 同じカテゴリの内容は1つのセクションにまとめる
- 新しい内容は既存カテゴリに追記、なければ新規カテゴリ作成
- **既存項目に新情報を追加する場合は上書きではなく統合アップデート**

**含める内容**:
- **基礎**: コマンド/概念の意味、前提知識
- **関連**: 類似機能との違い、使い分け
- **応用**: 実践的なパターン、ベストプラクティス
- **トラブル**: よくあるエラーと対処法

**目的**: 後から振り返って復習できるようにする
