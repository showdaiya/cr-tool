...
## 7) コマンド学習ログ（Command Learning Log）
学習目的のため、使用したPowerShell/Gitコマンドを**カテゴリ別**に記録する：
- 記録先: `docs/command-learning-log.md`
- フォーマット:
  ```markdown
  ## [カテゴリ名]（例: Git - ブランチ操作）
  ...
  **関連リンク**: [Git公式](https://git-scm.com/docs/git-checkout)
  ```
- 記録タイミング: 作業完了時、またはユーザーが「ログを書いて」と指示したとき
- 構成ルール:
  - 同じカテゴリのコマンドは1つのセクションにまとめる
  - 新しいコマンドは既存カテゴリに追記、なければ新規カテゴリ作成
  - **既存コマンドに新情報を追加する場合は上書きではなく統合アップデート**
- 含める内容:
  - **基礎**: コマンドの意味、前提知識
  - **関連**: 類似コマンドとの違い、使い分け
